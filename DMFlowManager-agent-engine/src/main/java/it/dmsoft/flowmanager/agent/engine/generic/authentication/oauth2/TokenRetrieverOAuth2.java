package it.dmsoft.flowmanager.agent.engine.generic.authentication.oauth2;
...
import it.dmsoft.flowmanager.agent.engine.generic.authentication.oauth2.interfacce.TokenOauth2Response;
...
public class TokenRetrieverOAuth2 implements Retriever<TokenRetrieverData> {

	@Override
	public TokenRetrieverData get(Logger logger) throws AuthenticationException {

		FailableFunction<ClientDataJWT, TokenRetrieverData> ff = new FailableFunction<ClientDataJWT, TokenRetrieverData>() {
			@Override
			public TokenRetrieverData apply(ClientDataJWT data, Logger logger) throws Exception {
				String token = null;
				...
				logger.info("Invocazione servizio di autenticazione con parametri recuperati da OAuth2Client");
				...
				map.put("scope", "archive");

				try {
					OcsGenericRestResponse<TokenOauth2Response> restResponse = Invoker.callEndpoint(url.toString(), null, HttpMethod.POST, headers, map, null, 0, false, TokenOauth2Response.class,
							logger);
					Throwable e = restResponse.getError();
					if (e != null) {
						logger.error("Errore richiesta token da servizio Oauth2", e);
						return new TokenRetrieverData(true, null);
					}
		...
		ClientDataJWT data = AuthenticationProperties.getData(NomeApplicazione.OAUTH2CLIENT, ClientDataJWT.class);
		if (data == null) {
